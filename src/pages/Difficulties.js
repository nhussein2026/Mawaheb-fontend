...
const DifficultyPage = () => {
    ...
    const fetchDifficulties = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/difficulties`,
          {
            headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/difficulties/${selectedDifficulty._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setDifficulties(
        difficulties.map((diff) =>
          diff._id === data.difficulty._id ? data.difficulty : diff
        )
      );
      setFormData({ title: "", description: "" });
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/difficulties/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
